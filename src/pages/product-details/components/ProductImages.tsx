...
const ProductImages = ({images, title}: ProductImagesProps) => {
  ...
  return (
    <div className="product-images col-span-4 max-md:mb-5 md:col-span-2 mx-auto">
      {/* --- Main Image */}
      <div className="product-details-card-image w-40 h-40 sm:w-60 sm:h-60 xl:w-80 xl:h-80 mx-auto">
        <div
      ...
      <div className="product-details-card-images flex gap-0.5">
        {images.map((img, index) => (
          <img
            ...
            key={index}
            className={clsx(
              "w-14 h-14 sm:w-18 sm:h-18 xl:w-20 xl:h-20 object-contain hover:border-gray-300 hover:p-0.5 transition-colors duration-150",
              currentImage === index
